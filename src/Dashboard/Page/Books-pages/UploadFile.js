...
export default class ModalComponent extends React.Component {
  ...
  toggle() {
    ...
    async uploadFile(file){
        ...
        var index=this.props.dataFromParent;
        return  await axios.post('http://localhost:8000/api/uploadfile-books/'+index, formData, {
            headers: {
  ...
  render() {
    ...
    return (
        <div>
        <Buton variant="outlined" size="small" color="primary" onClick={this.toggle}><AttachFileIcon /></Buton>
          ...
          <ModalHeader>Data ID: {this.props.dataFromParent}</ModalHeader>
            <ModalBody>
              <Row>
                <Col sm={10}>
                  <FormGroup>
                    <AttachFileIcon />{' '}: <img src={this.state.files} alt={this.state.files} width='500' />
                    <Label for="exampleFile">File</Label>
                    <CustomInput type="file" id="exampleCustomFileBrowser" name="files" onChange={this.onChange}/>
